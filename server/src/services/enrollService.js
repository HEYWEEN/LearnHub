...
export async function enrollCourse({ user, courseId }) {
  return withTransaction(async (conn) => {
    ...
    const [existing] = await repo.findEnrollment(conn, {userId:user.id, courseId});
    if (existing) {
    ...
    const [enrollment] = await repo.findEnrollment(conn, {userId:user.id, courseId});
...
export async function checkEnrollStatus({ user, courseId }) {
  const [existing] = await withConnection((conn) =>
    repo.findEnrollment(conn,{userId:user.id, courseId})
  );
...
export async function getUserEnrollments({ user }) {
  const [enrollments] = await withConnection((conn) =>
    repo.findEnrollment(conn, {userId:user.id})
  );
  return enrollments;
}
...
export async function getCourseEnrollments({ courseId }) {
  const enrollments = await withConnection((conn) =>
    repo.findEnrollment(conn,{ courseId} )
  );
  return enrollments;
}

export async function listEnrollments({user,courseId}) {
  const [enrollments,total] = await withTransaction(async (conn) =>Promise.all([
    repo.findEnrollment(conn, {instructorId:user.id, courseId}),
    repo.countEnrollment(conn, {instructorId:user.id, courseId})
  ]));
