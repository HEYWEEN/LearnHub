...
const register = asyncHandler(async (req,res)=>{
  ...
  const [rows] = await pool.query("INSERT INTO users (id, email, password, username, role) VALUES (UUID(),?, ?, ?, ?)", [email, password, username, role]);
  const [userRows] = await pool.query("SELECT id, email, username, role, created_At FROM users WHERE id = ?", [rows.insertId]);
